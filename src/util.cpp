...
const char * const BITCOIN_CONF_FILENAME = "charm.conf";
const char * const BITCOIN_PID_FILENAME = "charmd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "charm" is a composite category enabling all Charm-related debug output
            if(ptrCategory->count(string("charm"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CharmCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CharmCore
    // Mac: ~/Library/Application Support/CharmCore
    // Unix: ~/.charmcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CharmCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CharmCore";
#else
    // Unix
    return pathRet / ".charmcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty charm.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override charm.conf
